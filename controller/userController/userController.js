...
const { otpEmailHtml, reserEmailHtml } = require("../../assets/mails");
...
const sendResetMail = async (req, res) => {
  try {
    const { email } = req.body;
    const resetLink = `${process.env.CLIENT_BASE_URL}/resetpassword?email=${encodeURIComponent(email)}`;
    const data = await resend.emails.send({
      from: 'Fourcapedu Support <noreply@fourcapedu.com>',
      to: email,
      subject: 'Reset Your Fourcapedu Password',
      html: reserEmailHtml(resetLink),
    });
...
const sendOtp = async (req, res) => {
  try {
    ...
    await resend.emails.send({
      from: 'Fourcapedu Support <noreply@fourcapedu.com>',
      to: email,
      subject: 'Your Fourcapedu OTP Verification Code',
